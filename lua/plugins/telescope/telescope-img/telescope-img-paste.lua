...
local function enter_make(prompt_bufnr)
    local selected = action_state.get_current_picker(prompt_bufnr):_get_prompt()
    actions.close(prompt_bufnr)
    Create_Proj(selected)
end

local function enter(prompt_bufnr)
    local selected = action_state.get_selected_entry()
    if selected == nil then
        selected = {action_state.get_current_picker(prompt_bufnr):_get_prompt()}
    end
    actions.close(prompt_bufnr)
    Create_Proj(selected[1])
end
